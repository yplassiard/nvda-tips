...
class GlobalPlugin(globalPluginHandler.GlobalPlugin):
    ...
    def sayTip(self):
        ...
        if self.obj.role in ignoredRoles:
            return
        speech.speakMessage(_("You are on a %s: %s") %(controlTypes.roleLabels[self.obj.role], self.explain()))
    ...
    def explain(self):
        ...
        if self.obj.role in (controlTypes.ROLE_EDITABLETEXT, controlTypes.ROLE_RICHEDIT):
            ...
            if controlTypes.STATE_CHECKED not in self.obj.states:
                msg = _("Press space to se;ect this option, or use the arrow keys to select another one in this group.")
            else:
    ...
    def script_contextHelp(self, gesture):
        ...
        if obj.role is controlTypes.ROLE_BUTTON:
            if obj.parent and obj.parent.__class__.__name__ is 'NotificationArea':
                msg = _("You are in the notification area. This area contains icons of running programs and services allowing you to control their execution and settings. Use the left and right arrow keys to move beiween them and enter to activate the desired icon. You may also use the Application key (or context menu key) to open a pop-up menu containg more options regarding the currently focused icon. To quickly go to the notification area from anywhere, use the Windows+B shortcut.")
